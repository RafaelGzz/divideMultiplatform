...
class DeleteGroupUseCaseTest {
    ...
    private val useCase = DeleteGroupUseCase(mockGroupRepository, mockUserStateHolder)
    ...
    fun `should return success when group is deleted successfully`() = runTest {
        ...
        // When
        val result = useCase(groupId, image)

        // Then
        assertTrue(result is DeleteGroupUseCase.Result.Success)
        assertEquals(groupId, mockGroupRepository.receivedGroupId)
        assertEquals(image, mockGroupRepository.receivedImage)
        assertTrue(mockUserStateHolder.deleteGroupCalled)
