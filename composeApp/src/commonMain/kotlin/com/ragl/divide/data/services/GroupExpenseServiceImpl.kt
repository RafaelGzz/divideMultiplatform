...
class GroupExpenseServiceImpl : GroupExpenseService {
    ...
    private fun processExpenses(
        ...
        expenses.forEach { expense ->
            val (creditors, debtors) = expense.calculateDebtsAndPayers()
            debtors.forEach { (debtor, debtAmount) ->
                creditors.forEach { (creditor, _) ->
                    if (debtor != creditor) {
                        addDebt(balances, debtor, creditor, debtAmount)
                    }
                }
            }
        }
                ...
                for (secondaryCreditor in secondaryCreditors) {
                    ...
                    if (transferAmount > MINIMUM_DEBT) {
                        ...
                        updateOrRemoveDebt(
                            ...
                            debtor,
                            creditor,
                            debtToCreditor - transferAmount
                        )
                        updateOrRemoveDebt(
                            simplified,
                            creditor,
                            secondaryCreditor,
                            creditorToSecondary - transferAmount
                        )
     ...
     */
    override fun consolidateDebtsFromEvents(events: Collection<Event>): Map<String, Map<String, Double>> {
     ...
     */
    override fun consolidateDebtsFromEventsMap(eventsMap: Map<String, Event>): Map<String, Map<String, Double>> {
     ...
     */
    override fun getEventDebtsMap(eventsMap: Map<String, Event>): Map<String, Map<String, Map<String, Double>>> {
